  ...
  io.on("connection", (socket) => {
    ...
    socket.on("room:canvas:tool", ({ room, user, tool }) => {
      ...
      socket
        ...
        .emit("room:canvas:tool:done", { room, user, tool });
    });
    ...
    socket.on("room:canvas:layer-create", ({ room, user }) => {
      ...
      socket
        .to(socket.data.user.room)
        .emit("room:canvas:layer-create:done", { id });
    });
    ...
    socket.on("room:canvas:layer-delete", ({ room, user, id }) => {
      ...
      socket
        .to(socket.data.user.room)
        .emit("room:canvas:layer-delete:done", { room, user, id });
    });
    ...
    socket.on("room:canvas:layer-visible", ({ room, user }) => {
      ...
      socket
        .to(socket.data.user.room)
        .emit("room:canvas:layer-visible:done", { room, user });
    });
